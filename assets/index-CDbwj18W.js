(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const a of s)if(a.type==="childList")for(const o of a.addedNodes)o.tagName==="LINK"&&o.rel==="modulepreload"&&i(o)}).observe(document,{childList:!0,subtree:!0});function e(s){const a={};return s.integrity&&(a.integrity=s.integrity),s.referrerPolicy&&(a.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?a.credentials="include":s.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function i(s){if(s.ep)return;s.ep=!0;const a=e(s);fetch(s.href,a)}})();const d=["###############","#.....#...#...#","#.###.#.#.#.#.#","#o#...#.#...#o#","#.#.#####.###.#","#.#... ...#...#","#.### ###.#.###","#...#.G.G.#...#","###.#.###.#.#.#","#...#...#...#.#","#.#.###.#####.#","#o#....P....#o#","###############"],p=P(d),O=24,k=280,g={up:{row:-1,col:0},down:{row:1,col:0},left:{row:0,col:-1},right:{row:0,col:1}},T={up:"down",down:"up",left:"right",right:"left"},y={ArrowUp:"up",ArrowDown:"down",ArrowLeft:"left",ArrowRight:"right",w:"up",s:"down",a:"left",d:"right"},S=[{label:"Entrar en la web",href:"https://csilvasantin.github.io/"},{label:"Abrir el diario",href:"/diario/"}],E="AdmiraNeXT v.26.05.30.r4",G=["Si la home es tu puerta, el juego es tu contraseña emocional.","El diario se gana esquivando fantasmas, no haciendo click sin mirar.","Queremos más recreativa de barrio y menos splash page sin alma."];function x(l){new I(l).mount()}class I{root;state=v();tickHandle=null;audio=null;handleKeydown=t=>{const e=y[t.key]??y[t.key.toLowerCase()];e&&(t.preventDefault(),this.ensureAudio(),this.audio?.resume(),this.movePlayer(e))};constructor(t){this.root=t}mount(){window.addEventListener("keydown",this.handleKeydown),this.render()}ensureAudio(){this.audio||(this.audio=new C)}startLoop(){this.tickHandle===null&&(this.tickHandle=window.setInterval(()=>{this.advanceGhosts()},k))}stopLoop(){this.tickHandle!==null&&(window.clearInterval(this.tickHandle),this.tickHandle=null)}movePlayer(t){if(this.state.gameOver||this.state.unlocked)return;const e=this.tryMove(this.state.player,t);if(this.state.started=!0,this.state.playerDirection=t,this.startLoop(),this.audio?.setSirenActive(!0),!e){this.state.message="Pared. Gira y busca otra ruta.",this.audio?.playBump(),this.render();return}this.state.player=e;const i=this.state.grid[e.row][e.col];i==="."||i==="o"?(this.state.grid[e.row][e.col]=" ",this.state.pelletsLeft-=1,this.state.score+=i==="o"?250:100,this.audio?.playChomp(i==="o"),i==="o"?(this.state.frightenedTicks=O,this.state.message="Power pellet. Ahora mandas tú."):this.state.message="Bien. Sigue limpiando el laberinto."):this.state.message="No pares. Los fantasmas ya te han visto.",this.resolveCollisions(),this.state.pelletsLeft===0&&(this.state.unlocked=!0,this.state.message="Perfect run. Acceso desbloqueado.",this.stopLoop(),this.audio?.playUnlock(),this.audio?.setSirenActive(!1)),this.render()}advanceGhosts(){this.state.gameOver||this.state.unlocked||!this.state.started||(this.state.ghosts=this.state.ghosts.map(t=>this.moveGhost(t)),this.state.frightenedTicks>0&&(this.state.frightenedTicks-=1),this.resolveCollisions(),!this.state.gameOver&&!this.state.unlocked&&this.render())}moveGhost(t){const e=this.availableDirections(t),i=e.length>1?e.filter(r=>r!==T[t.direction]):e,s=i.length>0?i:e;let a=t.direction;s.length>0&&(a=this.state.frightenedTicks>0?s[Math.floor(Math.random()*s.length)]:this.pickChaseDirection(t,s));const o=this.tryMove(t,a);return o?{...t,direction:a,row:o.row,col:o.col}:{...t,direction:s[0]??t.direction}}pickChaseDirection(t,e){let i=e[0],s=Number.POSITIVE_INFINITY;return e.forEach(a=>{const o=this.tryMove(t,a);if(!o)return;const r=Math.abs(o.row-this.state.player.row)+Math.abs(o.col-this.state.player.col);r<s&&(s=r,i=a)}),i}availableDirections(t){return Object.keys(g).filter(e=>this.tryMove(t,e))}tryMove(t,e){const i=g[e],s=t.row+i.row,a=t.col+i.col,o=this.state.grid[s]?.[a];return!o||o==="#"?null:{row:s,col:a}}resolveCollisions(){const t=this.state.ghosts.findIndex(e=>e.row===this.state.player.row&&e.col===this.state.player.col);if(t!==-1){if(this.state.frightenedTicks>0){const e=this.state.ghosts[t];this.state.ghosts[t]={...e,row:e.home.row,col:e.home.col,direction:"left"},this.state.score+=400,this.state.message="Fantasma comido. Sigue presionando.",this.audio?.playGhostEaten();return}if(this.state.lives-=1,this.audio?.playDeath(),this.state.lives<=0){this.state.gameOver=!0,this.state.started=!1,this.state.message="Game over. El acceso sigue cerrado.",this.stopLoop(),this.audio?.setSirenActive(!1);return}this.state.player={...w},this.state.playerDirection="left",this.state.ghosts=L(),this.state.frightenedTicks=0,this.state.message=`Te han cazado. Quedan ${this.state.lives} vidas.`}}reset(){this.stopLoop(),this.audio?.setSirenActive(!1),this.state=v(),this.render()}toggleAudio(){this.ensureAudio(),this.audio?.resume(),this.audio?.toggleMute(),this.render()}render(){const t=p-this.state.pelletsLeft,e=this.state.grid.map((o,r)=>o.map((n,c)=>{const h=["maze-cell"];n==="#"?h.push("cell-wall"):n==="."?h.push("cell-dot"):n==="o"?h.push("cell-power"):h.push("cell-empty");const f=this.state.ghosts.find(m=>m.row===r&&m.col===c),A=this.state.player.row===r&&this.state.player.col===c;let u="";return A?(u=`<span class="player-core player-${this.state.playerDirection}"></span>`,h.push("cell-player")):f?(h.push("cell-ghost-host"),u=`<span class="ghost ${this.state.frightenedTicks>0?"ghost-frightened":""}" style="--ghost-color:${f.color}"><span class="ghost-eyes"></span></span>`):n==="."?u='<span class="dot"></span>':n==="o"&&(u='<span class="power"></span>'),`<div class="${h.join(" ")}">${u}</div>`}).join("")).join(""),i=S.map(o=>`
      ...
      `).join(""),s=G.map(o=>`<li>${o}</li>`).join(""),a=this.audio?!this.audio.isMuted():!0;this.root.innerHTML=`
